...
function execute_scalar($query,$params = array())
{
    return dbGetOne($query,$params);
}
function execute_row_assoc($query,$params = array())
{
    return dbGetRow($query,$params);
}
...
function get_image_path_by_index($id,$source = 0,$index = 0)
{
 $images = execute_scalar("SELECT image FROM images WHERE parentid = '$id' AND source = '$source'  ORDER BY showorder DESC LIMIT $index,1");
 if($images != "")
 {
  return "/images/files/$images";
 }
...
function get_content_info($parent=0, $where = "", $limit = '') {
    $arr = array();
     $sql=mysql_query("SELECT * FROM content WHERE parentid IN ($parent)  AND ispublish='1' $where ORDER BY showorder".($limit ? "
                        LIMIT 0,$limit " : '')." ");
...
function get_gallery($parent ='', $source = '', $is_main = '', $limit = '') {
    $arr = array();
     $sql=mysql_query("SELECT * FROM images WHERE parentid='$parent' AND source='$source' ".($is_main ? " AND is_main = '0'" : '')." ORDER BY showorder".($limit ? "LIMIT 0,$limit " : '')." ");
                while ($r=mysql_fetch_assoc($sql)){
                    array_push($arr, $r);
                }
     return $arr;
...
function getPageUrl($id,$table)
{
    return execute_scalar("SELECT url FROM $table WHERE id = '$id'");
}
function get_menu_item($where='', $menuId = "") {
    $arr = array();
     $sql=mysql_query("SELECT content.* FROM content LEFT JOIN userMenuItem ON content.id = userMenuItem.contentId  WHERE    content.ispublish='1' AND userMenuItem.menuId = '$menuId'  $where  ORDER BY showorder");
                while ($r=mysql_fetch_assoc($sql)){
                    array_push($arr, $r);
                }
...
{
        $sql_text = "SELECT parentid,urlname,name FROM content WHERE id = '$id'";
    if(!is_numeric($id))
    {
        $sql_text = "SELECT parentid,urlname,name FROM content WHERE urlname = '$id'";
    }
    $parent = execute_scalar($sql_text);
    
    if($parent != "0" && $parent != "35" && $parent != "41" && $parent != "36")
    {
...
{
    $sql = mysql_query("SELECT id FROM content WHERE parentid = '$parentid'");
    while($r = mysql_fetch_assoc($sql))
    {
...
function get_title()
{
    ...
    switch($content_type)
    {
        ...
        case "content":
            global $head;
            echo $head["pageTitle"] != "" ? $head["pageTitle"] : $head["name"];
            break;
        case "news":
            global $title;
            echo $title;
...
function get_path()
{
    ...
    switch($content_type)
    {
        case "content":
            ...
            reset($arr);
            ?>
             <li><a  href="<?=_SITE?>">Главная</a></li>
            <?
            foreach($arr as $key)
            {
                if ($key != $head['id']) {
              $urlname = execute_scalar("SELECT url FROM content WHERE id = $key");
              $path .= "<li><a href='"._SITE."".(  $urlname != "" ?   $urlname : $key )."'>".execute_scalar(is_numeric($key) ? "SELECT name FROM content WHERE id = '$key'" :"SELECT name FROM content WHERE urlname = '$key'" )."</a></li>";
            } else {
            $path .= "<li ><a class='active' >".execute_scalar(is_numeric($key) ? "SELECT name FROM content WHERE id = '$key'" :"SELECT name FROM content WHERE urlname = '$key'" )."</a></li>";    }
            }
            echo $path;
            break;    
        case "search": 
        ?>
            <li><a  href="<?=_SITE?>">Главная </a></li><li><a class="active">Результаты поиска по запросу: "<?=urldecode($_GET["stext"])?>"</a></li><?
            break;
            case "articles": 
            global $head;
        ?>
            <li><a  href="<?=_SITE?>">Главная </a></li> <li><a  href="/stati/">Статьи </a></li><li><a class='active' ><?=$head["name"]?></a></li><?
            break;  
        case "allquestion":
        ?>
           <a  href="<?=_SITE?>">Главная страница</a> &nbsp;»&nbsp; <a href="<?=_SITE?>questions.html">Опросы</a><? 
             break;
        default:
            global $head;
            $arr = array($head["id"]);
            ...
            reset($arr);
            ?>
             <a  href="<?=_SITE?>">Главная</a>
            <?
            foreach($arr as $key)
            {
              $urlname = execute_scalar("SELECT url FROM content WHERE id = $key");
              $path .= "  &nbsp;»&nbsp; <a href='"._SITE."".(  $urlname != "" ?   $urlname : $key )."'>".execute_scalar(is_numeric($key) ? "SELECT name FROM content WHERE id = '$key'" :"SELECT name FROM content WHERE urlname = '$key'" )."</a>";
            }
            echo $path;
            break;
    }
...
{
    include_once(_DIR.'inc/jsmin.php');
    return  JSMin::minify($contents);
}

function compressCSS($contents) 
{
...
function show_banner_by_ip()
{
    $user_ip = get_user_ip();
    $result = dbGetOne("SELECT COUNT(ip) as cnt FROM banner_show WHERE ip = '$user_ip'");
    return ( (int)$result == 1 ? false : true );
}

function show_banner_append_ip()
{
    $user_ip = get_user_ip();
    dbNonQuery("INSERT INTO banner_show (ip) VALUES ('$user_ip')");
}
